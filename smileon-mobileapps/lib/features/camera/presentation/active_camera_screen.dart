...
class ActiveCameraScreen extends StatefulWidget {
  const ActiveCameraScreen({super.key});

  @override
  State<ActiveCameraScreen> createState() => _ActiveCameraScreenState();
}

class _ActiveCameraScreenState extends State<ActiveCameraScreen> {
  ...
  Future<void> _takePicture() async {
    if (_cameraController != null && _cameraController!.value.isInitialized) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Foto ${_capturedPhotos.length}/4 berhasil diambil!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Nyalakan kamera terlebih dahulu untuk mengambil foto.'),
  ...
  void _showFrameSelectionSheet() {
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      constraints: const BoxConstraints(maxWidth: double.infinity), // Memastikan fullwidth meski di mode horizontal
      builder: (context) {
        return Container(
          width: double.infinity,
          height: MediaQuery.of(context).size.height * 0.9, // 90% dari layar horizontal
                    ...
                    const Center(
                      child: Text(
                        ...
                        style: TextStyle(
                          ...
                          shadows: [
                            Shadow(color: AppTheme.primaryRose, blurRadius: 20),
  ...
  Widget _buildWhiteOutlinedButton(String text, IconData icon, {VoidCallback? onTap}) {
        ...
        builder: (context, constraints) {
          ...
          return Container(
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isPink ? const Color(0xFFF0DDE2) : const Color(0xFFD6E4F5),
      ...
      child: SizedBox(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: (isPink ? const Color(0xFFB54668) : const Color(0xFF1E5296))
                        ...
                        child: Center(
                          child: Text(
                            ...
                            style: TextStyle(
                              color: themeColor,
                              fontSize:
                                  isMini ? (height * 0.30) : (height * 0.24),
  ...
  Widget _buildPhotostripFooter(
    ...
    final heartColor =
        isPink ? const Color(0xFFF06292) : const Color(0xFF2563EB);
    final lineColor =
        isPink ? const Color(0xFFF8BBD0) : const Color(0xFF93C5FD);
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color:
              widget.color ??
              (widget.isPrimary
                  ? (_isPressed ? Colors.white : Colors.white.withValues(alpha: 0.6))
                  : (_isPressed
...
class _PhotostripFloralPainter extends CustomPainter {
  ...
  void _paintBlueTheme(Canvas canvas, Size size, double w, double h) {
    ...
    canvas.drawPath(
      ...
      Paint()
        ...
        ).createShader(Rect.fromCircle(center: heartCenter1, radius: w * 0.075)),
    );
    // Kilau 3D pada hati
    canvas.drawCircle(
    ...
    _drawDahliaFlower(canvas, dahliaCenter, w * 0.13, blueDeep, blueLight, bluePale);
    ...
    canvas.drawPath(
      ...
      Paint()
        ..shader = LinearGradient(
          ...
          colors: [const Color(0xFF4A89DD), const Color(0xFF17427E)],
        ).createShader(Rect.fromCircle(center: heartCenter2, radius: w * 0.065)),
    );
    canvas.drawCircle(
  ...
  void _paintPinkTheme(Canvas canvas, Size size, double w, double h) {
    ...
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(butterflyCenter.dx - w * 0.03, butterflyCenter.dy - h * 0.005),
        width: w * 0.07,
        height: w * 0.05,
      ),
      petalPaint2,
    ...
    canvas.drawCircle(Offset(w * 0.04, gap2Y + gap * 0.15), w * 0.04, petalPaint1);
